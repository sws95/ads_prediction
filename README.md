...
**평가 원칙.** 입찰 파라미터(ORTB λ, Linear base, Const c0)는 train(6/06–6/11)에서 결정해 test(6/12)에 적용한다. test에서 직접 λ를 고른 것은 미래를 본 것이라 **oracle(상한선)**으로만 쓴다. λ는 그리드 대신 **bisection**으로 푼다(비용이 λ에 단조, Zhang 2014 방식). win rate 모델은 imp 로그 기반 **CDF(비모수)**와 **b/(b+c)(파라메트릭)** 둘을 비교한다.
